...
################################################
	...
	Func Insert 
		...
		cValues = ""
		for x in aColumns							# like sara,ahmed in coulumn{names}
			cValues += "'" + EscapeString(aPageVars[x]) + "',"		# we add to only data entery without indexes like 'Mahmoud',15000i,  
		Next
		cValues = left(cValues,len(cValues)-1)					# remove last comma in the last column value ex in table names 'sara,'saly', so we remove comma		
		cColumns = ""
		for x in aColumns							# separate values in columns by comma ,
			cColumns += x + ","						# we add "" , for every index like 1,2,3,4,5,... but it is string
		next
		cColumns = left(cColumns,len(cColumns)-1)  				# to remove last comma	in indexes
		query("insert into " + cTableName + "("+cColumns+") values (" + 	# to add   values to columns names
				 cValues + ")" )
...
################################################
	...
	func ShowTable
		...
		if ( nActivePage > nPagesCount ) and nRecordsCount > 0 		# to show next last prev when more than one page
			ErrorMsg(cPageError)
			return
		ok
...
##################################################### show style for the page if doesn't has any  record #####################################################
					else
					...
					divend()
#####################################################					
					divstart([ :style = stylewidth("100%") + styleheight("10%") ])				# add record button
						button([ :value = oTranslation.cAddRecord, :onclick = "myadd()" , :style = stylegradient(20) ]) 		 # myadd() is defined in the salary file as it has attributes that may be changed by user demand
					divend()

					divstart([ :id = "mysubpage" , :style = stylewidth("100%")+stylemargintop("2%")+styleheight("70%") ])  # page that appeared when press add record
					divend()					
				divend()
			divend()
			divend()
			...
			Script( oThisView.AddFuncScript(self,oController) + 
				...
				scriptfuncselect("selectrecord",oTranslation.comboitems,
						 ...
						 oController.cMainURL+oController.cOperation+"=edit&"+oController.cRecID+"=",  		# =0
						 oController.cMainURL+oController.cOperation+"=delete&"+oController.cRecID+"=") )	 # =0	
